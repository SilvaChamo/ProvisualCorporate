...
export default function Dashboard() {
  ...
  const getAllowedClientRootFolders = (allFolders: FolderData[]): FolderData[] => {
    ...
    // A pasta do cliente é identificada pelo clientEmail igual ao email do utilizador
  ...
  return (
    <div className="flex h-screen bg-[#fafafa] text-gray-800 font-sans overflow-hidden">
      ...
      >
        <div className="p-4 overflow-y-auto custom-scrollbar flex-1">
          ...
          <nav className="space-y-0.5">
            ...
            {userProfile?.role === 'admin' && (
              <>
                <div className={cn("my-5", sidebarCollapsed && "my-3 lg:my-5")} />
                <h3
                  className={cn(
                    "text-[13px] font-black text-[#a21b7e] uppercase tracking-[0.08em] px-3 py-2.5 mb-2 bg-[#a21b7e]/5 rounded-sm",
                ...
                <DashboardSidebarItem
                  ...
                  onClick={() => { setActiveTab('google_drive'); handleGoogleSync(arquivoFolderId || 'root'); setVisibleImagesCount(IMAGES_GRID_PAGE_SIZE); }}
                />
                <DashboardSidebarItem
                  collapsed={sidebarCollapsed}
                  icon={<Users size={20} />}
                  ...
                  onClick={() => { setActiveTab('google_drive'); handleGoogleSync(undefined, 'sharedWithMe'); setVisibleImagesCount(IMAGES_GRID_PAGE_SIZE); }}
                />
                <DashboardSidebarItem
                  collapsed={sidebarCollapsed}
                  icon={<Clock size={20} />}
                  ...
                  onClick={() => { setActiveTab('google_drive'); handleGoogleSync(undefined, 'recent'); setVisibleImagesCount(IMAGES_GRID_PAGE_SIZE); }}
                />
                <DashboardSidebarItem
                  collapsed={sidebarCollapsed}
                  icon={<Star size={20} />}
                  ...
                  onClick={() => { setActiveTab('google_drive'); handleGoogleSync(undefined, 'starred'); setVisibleImagesCount(IMAGES_GRID_PAGE_SIZE); }}
                />
                <DashboardSidebarItem
                  collapsed={sidebarCollapsed}
                  icon={<Trash2 size={20} />}
                  ...
                  onClick={() => { setActiveTab('google_drive'); handleGoogleSync(undefined, 'trashed'); setVisibleImagesCount(IMAGES_GRID_PAGE_SIZE); }}
                />
              </>
            )}
          </nav>
        ...
        <div className={cn("bg-[#a21b7e]/5 border-t border-[#a21b7e]/10 shrink-0", sidebarCollapsed ? "p-2" : "p-4")}>
          <div className={cn("flex items-center gap-2 mb-2 text-xs font-bold text-[#a21b7e]", sidebarCollapsed ? "justify-center mb-0" : "")}>
            <Cloud size={16} />
            <span className={cn(sidebarCollapsed && "hidden")}>Armazenamento</span>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        {activeTab !== 'site_home' && (
        <div className="bg-white px-4 py-2 md:px-8 md:py-3 flex items-center justify-between border-b border-gray-100 z-10">
          <div className="hidden md:flex items-center gap-2 text-sm font-medium min-w-0 flex-1">
            {selectedAsset ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex h-9 bg-gray-50 p-1 rounded-sm border border-gray-100 items-center shrink-0">
              <button
                              ...
                              <div className="flex items-center gap-3 truncate flex-1 min-w-0">
                                ...
                                <div className="truncate min-w-0">
                                  ...
                                  {(folder as any).clientEmail && userProfile?.role === 'admin' && (
                                    <span className="text-[10px] font-medium text-[#a21b7e] flex items-center gap-1.5 mt-0.5 min-w-0">
                                      <AccountAvatar
                                        size="xs"
                                        displayName={
                                          findAccountByEmail(accounts, (folder as any).clientEmail)?.displayName
                                        }
                                        email={(folder as any).clientEmail}
                                      />
              ...
              <div className="p-5">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                  {accounts.filter(a => a.role !== 'admin').map(client => {
                    const parsed = parseAccountDisplay(client.displayName || "");
                    ...
                    return (
                      ...
                      >
                        <AccountAvatar
                          ...
                          displayName={client.displayName}
                          email={client.email}
                          active={selectedClientId === client.email}
                        />
                        <div>
            ...
            >
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto max-h-[300px] px-2 pb-2 custom-scrollbar">
                  ...
                  {folders
                    .filter(f => f.id !== item.id && f.id !== currentParentId && !(f as any).trashed && !(f as any).deleted && (organizarSearch === '' || f.name.toLowerCase().includes(organizarSearch.toLowerCase())))
                    .map(f => (
                      <button
                        key={f.id}
                        onClick={async () => {
