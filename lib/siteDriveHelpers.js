...
export async function listGalleryAlbumsOffline(supabase) {
  ...
  const albums = meta.map((entry) => {
    const staticImage = STATIC_GALLERY_IMAGES[entry.slug] || "";
    const merged = {
      slug: entry.slug,
      ...
      coverUrl: staticImage || null,
      coverDriveId: entry.coverDriveId || null,
      photoCount: entry.photoCount ?? 0,
    };
    const driveImage = albumCoverImageUrl(merged);
    return {
      ...merged,
      image: driveImage || staticImage,
    };
  });

  return { siteFolderId: null, albums };
}
