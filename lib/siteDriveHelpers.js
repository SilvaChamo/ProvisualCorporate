...
export async function listSiteGalleryAlbumsSummary(drive, supabase) {
  ...
  const albums = albumFolders.map((folder) => {
    ...
    return {
      ...
      coverUrl: metaEntry?.coverDriveId ? driveThumbUrl(metaEntry.coverDriveId) : merged.coverUrl,
      photoCount: metaEntry?.photoCount ?? merged.photoCount ?? 0,
      image: albumCoverImageUrl({
        ...merged,
        coverDriveId: metaEntry?.coverDriveId || merged.coverDriveId,
      }),
    };
  });
...
/** Resolve imagem pelo índice do Drive; ignora IDs antigos guardados no Supabase. */
...
export async function resolveHomeContentImages(drive, supabase, content) {
  ...
  // Sempre revalidar contra o Drive (IDs antigos em /api/drive/ deixam de funcionar).
  ...
  const eventTypes = (base.eventTypes?.length ? base.eventTypes : HOME_DEFAULT_FILES.eventTypes.map(() => ({}))).map(
    (item, i) => ({
      ...item,
...
export async function createSiteGalleryAlbum(drive, supabase, { title, subtitle, cover, photos = [] }) {
  ...
  const albumData = await listSiteGalleryAlbumPhotos(drive, supabase, slug);
  const coverPhoto =
    albumData.photos.find((p) => /^cover\./i.test(p.name || "")) ||
    albumData.photos.find((p) => /cover/i.test(p.name || "")) ||
    albumData.photos[0];
...
export async function updateSiteGalleryAlbum(
  ...
  const folder = await findGalleryAlbumFolder(drive, supabase, slug);
...
export async function deleteSiteGalleryPhoto(drive, supabase, slug, photoId) {
  ...
  const photo = children.find((file) => file.id === photoId);
